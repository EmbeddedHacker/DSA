...
int main(void)
{
    ...
    printf("All assertionos on the empty list are successful\n");
    puts(line);
    ...
    show_list(p_list,"Showing list to demostrate that get_start() returns the end data without removing it:");
    puts(line);
    ...
    assert(status == LIST_DATA_NOT_FOUND);
    printf("Expected error in removing non-existent data 70\n");
    puts(line);

    status = remove_data(p_list, 0);
    assert(status == SUCCESS);
    show_list(p_list,"Showing list after removing data 0:");
    puts(line);

    status = find(p_list, 91);
    assert(status == FALSE);
    printf("Expected return value FALSE find() for non-existent data 91\n");
    puts(line);

    status = find(p_list, 30);
    assert(status == TRUE);
    printf("Expected retrun vlaue TRUE find() for existing data 30\n");
    puts(line);
...
int insert_after(struct node* p_list, int existing_data, int new_data)
{
    ...
    // Step 1 : search for the node containig the first occurance of the existing data
    run = p_list;
    while(run != NULL)
    {
...
int insert_before(struct node* p_list, int existing_data, int new_data)
{
    ...
    // step 1 : Search for the firt occurance of the existing data
